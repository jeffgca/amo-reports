...
function main(builder_conf, amo_conf, callback) {
    ...
    var daily_sql = 'SELECT COUNT(id) AS count FROM jetpack_package WHERE author_id != 21322 AND DATE(created_at) = ?';
    var monthly_sql = 'SELECT COUNT(id) AS count FROM jetpack_package WHERE author_id != 21322 AND DATE(created_at) >= ? AND DATE(created_at) < ?';
        ...
        function(p_callback) {
            ...
            async.whilst(
                ...
                function(_callback) {
                    ...
                    (function(_date) {
                        builder_conn.query(daily_sql, [_date], function(err, rows, fields) {
                            if (err) throw err;
                            // D(rows);
        ...
        function(p_callback) {
            ...
            async.whilst(
                ...
                function(_callback) {
                    ...
                    (function(start, end) {
                        // L(start, end);
                        builder_conn.query(monthly_sql, [start, end], function(err, rows, fields) {
                            // D(rows);
                            if (err) throw err;
...
if (!module.parent) {
  ...
  main(builder_config, amo_config, function(err, results) {
    ...
    csv().from(monthly).to.path('./monthly_builder_creation.csv');
    csv().from(daily).to.path('./daily_builder_creation.csv');
  });
}
